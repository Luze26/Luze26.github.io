...
class PerlinCharsSketch extends React.PureComponent {
  ...
  getColor() {
    ...
    switch (colorsConfig.mode) {
      ...
      case ExtendedColor.MODES.PALETTE:
        color = colorUtils.getPaletteColor(colorsConfig.colors, random.value()).hexString;
        break;
      case ExtendedColor.MODES.GRADIENT:
        color = colorUtils.getHexStringFromHex(
          colorUtils.getGradientColor(colorsConfig.colorStops, random.value()),
        );
        break;
      default:
        color = 0;
  ...
  init() {
    this.points = this.createGrid().filter(() => random.value() > this.props.config.hiddenChance);
